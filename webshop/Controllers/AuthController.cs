...
public class AuthController : ControllerBase
{
    ...
    public IActionResult Login([FromBody] LoginDto data)
    {
        try
        {
            if (data == null || string.IsNullOrWhiteSpace(data.Email) || string.IsNullOrWhiteSpace(data.Password))
                return BadRequest(new { success = false, error = "Email és jelszó megadása kötelezõ" });
    ...
    public IActionResult Logout() => Ok(new { success = true, message = "Sikeres kijelentkezés" });
}
