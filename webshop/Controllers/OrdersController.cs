...
public class OrdersController : ControllerBase
{
    ...
    private async Task<IActionResult> CreateOrder(System.Data.IDbConnection conn)
    {
        ...
        if (data == null)
            return Ok(new { success = false, error = "Hibas JSON adat" });
        ...
        if (cartItems.Count == 0)
            return Ok(new { success = false, error = "A kosar ures" });
        ...
        try
        {
            await conn.ExecuteAsync(@"
                ...
                VALUES (@UserId, @RSz, @Nev, @Email, @Tel, @Ir, @Var, @Utca, @Hsz, @Megj, @Ossz, @Szall, @Veg, 'utanvet', 'fuggoben')",
                new { UserId = userId, RSz = rendelesSzam, Nev = nev, Email = email, Tel = telefon, Ir = iranyitoszam, Var = varos, Utca = utca, Hsz = hazszam, Megj = megjegyzes, Ossz = osszeg, Szall = szallitasiDij, Veg = vegosszeg },
                transaction);
            ...
            foreach (var rawItem in cartItems)
            {
                ...
                int? aId = r["alkatresz_id"] is not DBNull && r["alkatresz_id"] != null ? Convert.ToInt32(r["alkatresz_id"]) : null;
                int? oId = r["olaj_id"] is not DBNull && r["olaj_id"] != null ? Convert.ToInt32(r["olaj_id"]) : null;
                if (aId == 0) aId = null;
                if (oId == 0) oId = null;
                ...
                await conn.ExecuteAsync(@"
                    ...
                    new
                    {
                        ...
                        AId = aId,
                        OId = oId,
                        TNev = r["termek_nev"]?.ToString() ?? "Ismeretlen termek",
                        Menny = menny,
                        Ar = ar,
                        Ossz = itemOsszeg
                    },
    ...
    private async Task<IActionResult> GetOrder(System.Data.IDbConnection conn)
    {
        ...
        if (orderId == 0)
            return Ok(new { success = false, error = "Hianyzo rendeles ID" });
    ...
    private async Task<IActionResult> UpdateOrderStatus(System.Data.IDbConnection conn)
    {
        ...
        if (orderId == 0 || string.IsNullOrEmpty(statusz))
            return Ok(new { success = false, error = "Hianyzo adatok" });
